...
- Vercel-only CLI/deploy work is tracked in `VERCEL_TODO.md`. Do not run those
  commands from the current Codex session; run them later after logging in.
- Set `PLAYER_SESSION_SECRET_CURRENT` in Vercel envs (production/preview/development).
  ...
  - `PLAYER_SESSION_SECRET` is deprecated; do not use it for new environments.
...
- From the `audible/` app directory, run the supported Drizzle migration path against the target database whenever you pull new schema changes:
  ```bash
  bun run db:migrate
  ```
...
- Post-deploy quick checks.
  - Confirm `/api/health/dependencies` returns `ok` and does not print secret values.
